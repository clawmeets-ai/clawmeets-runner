...
name: personalize
description: >
  Worker-side counterpart to /clawmeets:interview. Triggered by a
  personalize-trigger DM (`<!-- clawmeets:personalize-trigger -->`) — typically
  posted by the "Personalize" button at the bottom of a worker's DM input.
  Read what's known about the user from USER.md, judge whether 1–3 user-
  specific facts are still missing that would substantially raise the
  quality of a tailored field-knowledge dump (e.g. industry + geography
  for a career coach), ask only those, then reply with a ~1500–3000 word
  field-knowledge dump anchored on what's now known. Worker-only — the
  user's assistant runs /clawmeets:interview instead.
---
...
## You only run if you are a worker

The user's personal assistant — the agent named `{username}-assistant` —
runs `/clawmeets:interview`, not this skill. Compare your name (in the
`== AGENT ID ==` block of your prompt) against that pattern.

- **If your name ends with `-assistant`**: reply with one line —
  "personalize-trigger is for workers; I run /clawmeets:interview for
  USER.md" — and stop. Don't echo the marker.
- **If you are a worker** (everyone else): continue.

## Step 1 — read what you already know

Read `$CLAWMEETS_AGENT_DIR/memory/USER.md` if it exists (it may not —
the user might not have run the assistant's interview yet, and that's
...
1. Reply with a **~1500–3000 word field-knowledge dump** in the DM —
   ...
   you now know about them; don't drift into a generic primer.
...
- **Run-scoped only.** The answers the user gives in turn 2 inform
  *this* dump. **Do not write `USER.md`** — that's assistant-owned and
  the user explicitly bootstraps it through `/clawmeets:interview`.

...
name: interview
description: >
  Multi-turn DM interview that fills out USER.md from a resume upload,
  pasted bio text, or (best-effort) public profile URLs. Triggered by an
  interview-trigger DM (`<!-- clawmeets:interview-trigger -->`) — typically
  posted by the Welcome page's "Personalize Assistant NOW" button on
  first run. Assistant-only (workers do not own USER.md). Ask for the
  easiest input first, mine what arrives, ask only what's still missing
  AND important, then write USER.md and stop.
---
...
## You only run if you are the user's personal assistant

The user's personal assistant is the agent named `{username}-assistant`.
Compare your name (in the `== AGENT ID ==` block of your prompt)
against that pattern.

- **If you are the assistant**: continue.
- **If you are a worker** (everyone else): reply with one line —
  "interview-trigger is for the user's assistant; I don't own
  USER.md" — and stop.
...
- If it exists: reply with one line — "Already bootstrapped —
...
3. **Append** `## [YYYY-MM-DD] bootstrap | interview` to
...
4. **Reply** in the DM with a 3-bullet summary of what you captured —
   plain prose, no headings. Start the reply with `Bootstrapped~`.
   **Do not** include the marker in this final reply (its absence
   signals "interview complete" to your future-self on any later DM).
...
"interview in progress", **end every interview reply with the marker
comment** until you write `USER.md`. The marker stays as an HTML
...
marker on the "Incoming message from {user}" block of every interview
turn.

```
<!-- clawmeets:interview-trigger -->
```

Drop the marker on the final "Bootstrapped~" reply. From that turn
on, the runner stops re-prepending and any further DMs are normal
chat (not interview continuations).
...
- **Do not write to `learnings/`** beyond the single log line in
  step 4. Lessons accumulate via scheduled reflection, not bootstrap.

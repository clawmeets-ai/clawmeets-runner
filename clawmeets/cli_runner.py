...
def agent_run(
    ...
    asyncio.run(_runner_loop(agent_name, agent_id, token, server, Path(agent_dir), working_dir, knowledge_dir, git_url, git_ignored_folder))
...
async def _runner_loop(
    ...
    cli = ClaudeCLI(action_schema=action_schema)
